...
class TwoJacoPushGCEnv(TwoJacoEnv):
    def __init__(self, **kwargs):
        ...
        self._env_config.update({
            ...
            "target_pos": [np.random.uniform(-0.25,0.25), np.random.uniform(-0.25,0.25), 0.86],
            "ctrl_reward": 1e-4,
    ...
    def add_goals(self, ob):
        ...
        obs_and_goals['achieved_goal'] = np.array(self._get_pos('cube1')) if self._env_config["train_left"] else np.array(self._get_pos('cube2'))
        obs_and_goals['desired_goal'] = np.array(self._env_config['target_pos'])
    ...
    def reset(self):
        ...
        # goal2_index = self.sim.model.geom_name2id("goal2")
        # self.sim.data.geom_xpos[goal1_index] = new_goal
        self.sim.model.geom_pos[goal1_index] = new_goal
        # self.sim.data.geom_xpos[goal2_index] = new_goal
        self.sim.step()
        ob = super().reset()
