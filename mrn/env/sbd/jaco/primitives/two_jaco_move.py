...
class TwoJacoMoveEnv(TwoJacoEnv):
    def __init__(self, **kwargs):
        self.name = 'two-jaco-move'
        super().__init__('two_jaco_move.xml', **kwargs)
        ...
        self._env_config.update({
            ...
            "move_reward": 500,
            "move_target": np.array([0.5, 0.5, 0.86]),
            "dist_reward": 100,
    ...
    def _compute_reward(self, count_hold=False):
        ...
        for i in range(2):
            ...
            # target_distances_x = [
            #     float(np.abs(cube1_pos[0] - self._env_config['move_target'][0])),
            #     float(np.abs(cube2_pos[0] - self._env_config['move_target'][0]))
            # ]
            # target_distances_y = [
            #     float(np.abs(cube1_pos[1] - self._env_config['move_target'][1])),
            #     float(np.abs(cube2_pos[1] - self._env_config['move_target'][1]))
            ...
            if in_hands[i] and in_air[i]:
                # move_rewards[i] = (-self._env_config["move_reward"] * target_distances_xy[i] *0 - self._env_config["move_reward"] * target_distances_z[i]) / 1
                move_rewards[i] = self._env_config["move_reward"] * target_heights[i]

                if count_hold and target_heights[i] > 0.8:
                    self._hold_duration[i] += 1
                if self._hold_duration[i] >= self._env_config['hold_duration']:
                    print('Cube {}: success move!'.format(i + 1))
                elif count_success:
                    self._success = False
            elif count_success:
                self._success = False
        ...
        info = {"reward_dist": organize_info(dist_reward),
                "reward_move": organize_info(move_rewards),
