...
class TwoJacoPlaceGCEnv(TwoJacoEnv):
    ...
    def _step(self, a):
        ...
        obs_and_goals['achieved_goal': self._get_pos('cube1') if self._env_config["train_left"] else self._get_pos('cube2')]
        obs_and_goals['desired_goal': self._env_config['dest_pos']]
    ...
    def reset(self):
        self._env_config.update(
            {
                'dest_pos': [np.random.uniform(0,0.5), np.random.uniform(-0.5,0.5), np.random.uniform(0,1)]
            }
        )
        return super().reset()
